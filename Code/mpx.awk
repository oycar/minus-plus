  ...
  if (2 == number_accounts) {
    ...
    if ($i ~ /^[[:upper:]]{3}$/) {
      ...
      if (Transaction_Currency in Price)
        ...
        assert((0), "No exchange rate available for <" Transaction_Currency "> at <" get_date(now) ">")
...
function is_open(a, now,     p) {
  ...
  if (((a) ~ /^ASSET\.(CAPITAL|FIXED|CURRENCY)[.:]|^EQUITY[.:]/))
...
function initialize_account(account_name,    class_name, array, p, n,
  ...
  if (((account_name) ~ /^ASSET\.(CAPITAL|FIXED|CURRENCY)[.:]|^EQUITY[.:]/)) { # This could include EQUITY too
...
function split_account(now, a, b, split_factor,
  ...
  assert(((a) ~ /^ASSET\.(CAPITAL|FIXED|CURRENCY)[.:]|^EQUITY[.:]/), "split_account: <" a "> not a unitized account")
...
function filter_array(now, data_array, name, show_blocks,
  ...
  for (a in Leaf)
    if ((a in data_array) && ((a) ~ /^ASSET\.(CAPITAL|FIXED|CURRENCY)[.:]|^EQUITY[.:]/))
...
function print_gains(now, past, is_detailed, gains_type, reports_stream, sold_time, show_capital,
  ...
  for (a in Leaf) {
    ...
    if ((show_capital && ((a) ~ /^ASSET\.CAPITAL[.:]/)) || (!show_capital && ((a) ~ /^ASSET\.CURRENCY[.:]/))) {
...
function get_capital_gains(now, past, is_detailed,
    ...
    reports_stream = (("CT" ~ /[cC]|[aA]/ && "CT" !~ /[zZ]/)?( ((!Show_FY || ((now) == Show_FY))?( "/dev/stderr"):( "/dev/null"))):( "/dev/null"))
...
function print_operating_statement(now, past, is_detailed,     reports_stream,
  ...
  # The reports_stream is the pipe to write the schedule out to
  reports_stream = (("CT" ~ /[oO]|[aA]/ && "CT" !~ /[zZ]/)?( ((!Show_FY || ((now) == Show_FY))?( "/dev/stderr"):( "/dev/null"))):( "/dev/null"))
...
function print_balance_sheet(now, past, is_detailed,    reports_stream,
  ...
  # The reports_stream is the pipe to write the schedule out to
  reports_stream = (("CT" ~ /[bB]|[aA]/ && "CT" !~ /[zZ]/)?( ((!Show_FY || ((now) == Show_FY))?( "/dev/stderr"):( "/dev/null"))):( "/dev/null"))
  ...
  class_list["LIABILITY.CURRENCY"] = (1)
...
function get_market_gains(now, past, is_detailed,    reports_stream) {
  # Show current gains/losses
   # The reports_stream is the pipe to write the schedule out to
   reports_stream = (("CT" ~ /[mM]|[aA]/ && "CT" !~ /[zZ]/)?( ((!Show_FY || ((now) == Show_FY))?( "/dev/stderr"):( "/dev/null"))):( "/dev/null"))
...
function print_depreciating_holdings(now, past, is_detailed,      reports_stream, a, p, open_key, close_key, parcel_depreciation, account_depreciation,
  ...
  # The reports_stream is the pipe to write the schedule out to
  reports_stream = (("CT" ~ /[dD]|[aA]/ && "CT" !~ /[zZ]/)?( ((!Show_FY || ((((now) - 1)) == Show_FY))?( "/dev/stderr"):( "/dev/null"))):( "/dev/null"))
  if ("/dev/null" == reports_stream)
    return
...
function print_dividend_qualification(now, past, is_detailed,
  ...
  reports_stream = (("CT" ~ /[qQ]|[aA]/ && "CT" !~ /[zZ]/)?( ((!Show_FY || ((now) == Show_FY))?( "/dev/stderr"):( "/dev/null"))):( "/dev/null"))
...
function income_tax_aud(now, past, benefits,
  ...
  # Print this out?
  write_stream = (("CT" ~ /[tT]|[aA]/ && "CT" !~ /[zZ]/)?( ((!Show_FY || ((now) == Show_FY))?( "/dev/stderr"):( "/dev/null"))):( "/dev/null"))
...
function imputation_report_aud(now, past, is_detailed,
  ...
  # The reports_stream is the pipe to write the schedule out to
  reports_stream = (("CT" ~ /[iI]|[aA]/ && "CT" !~ /[zZ]/)?( ((!Show_FY || ((now) == Show_FY))?( "/dev/stderr"):( "/dev/null"))):( "/dev/null"))
...
function get_parcel_gains_aud(a, p, now,            held_time, gains, parcel_gains, units) {
  ...
  # Total gains (accounting gains)
  gains = (Parcel_Proceeds[a][ p]) + sum_cost_elements(Accounting_Cost[a][p], now) # All elements
  ...
  if ((((gains) - ( -Epsilon)) < 0)) {
    # Adjustments are negative and reduce taxable gains
    if (held_time >= 31622400)
      parcel_gains = (1.0 - ((CGT_Discount[2])?( (CGT_Discount[1]/CGT_Discount[2])):( assert((0), "Division by zero in rational fraction" CGT_Discount[1] "/" CGT_Discount[2])))) * gains
    else
      parcel_gains = gains
  }
...
function parse_transaction(now, a, b, amount,
  ...
  if (units < 0) {
    ...
    if (((a) ~ /^ASSET\.(CAPITAL|FIXED|CURRENCY)[.:]|^EQUITY[.:]/) || ((b) ~ /^ASSET\.(CAPITAL|FIXED|CURRENCY)[.:]|^EQUITY[.:]/)) {
      assert(!((a) ~ /^ASSET\.(CAPITAL|FIXED|CURRENCY)[.:]|^EQUITY[.:]/) ||  !((b) ~ /^ASSET\.(CAPITAL|FIXED|CURRENCY)[.:]|^EQUITY[.:]/),
    ...
    } else if (((b) ~ /^(ASSET|LIABILITY)\.TERM[.:]/) || ((b) ~ /^(ASSET|LIABILITY)\.CURREN(T|CY)[.:]/)) {
...
function checkset(now, a, account, units, amount, is_check,
  ...
  if (is_check) {
    ...
    switch(action) {
      ...
      case "PRICE" :
        assert(((account) ~ /^ASSET\.(CAPITAL|FIXED|CURRENCY)[.:]|^EQUITY[.:]/), sprintf("CHECK/SHOW: Only assets or equities have a PRICE: not %s\n", (Leaf[account])))
...
function sell_units(now, ac, u, x, parcel_tag, parcel_timestamp,        du, p, did_split, new_price, proportional_cost, catch_up_depreciation, t, key) {
  ...
  assert(((((u) - ( Epsilon)) <= 0) && (((u) - ( -Epsilon)) >= 0)), sprintf("sell_units: Failed to sell the requested %f units of %s", u, (Leaf[ac])))
